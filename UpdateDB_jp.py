...
def update_idoldata(cls_name):
    ...
    extract_end = findall("""<a class="card-select" href="/cards/([0-9]+?)">""", main_page_data)
    extract_end.sort(reverse=True)
    extract_end = int(extract_end[0]) + 1
    namedict = dict()

    try:
        extract_start = int(cur1.execute('select max(idnumber) from IdolDB').fetchone()[0]) + 1
        db_exist = True
    except:
        extract_start = 1
        db_exist = False
    ...
    if db_exist == True and txt_last == extract_end and extract_start == extract_end:
        cls_name.config_info("Checking Main DB and mltdkei_info.txt Updates... No Updates Found.")
        return
    ...
    if db_exist == False:
        ...
        for idnumber in ssr_list:
            ...
            maxrank = findall('最大マスターランク</span> (.+?)</p></div>', webdata)[-1]
            ...
            if maxrank == old_maxrank:
                ...
                updateg_count += 1
                vocal = findall('ボーカル</span> (.+?)</p></div>', webdata)[-1]
                dance = findall('ダンス</span> (.+?)</p></div>', webdata)[-1]
                visual = findall('ビジュアル</span> (.+?)</p></div>', webdata)[-1]
                total = findall('総アピール</span> (.+?)</p></div>', webdata)[-1]
                skill = findall('スキル説明</span> (.+?)</p></div>', webdata)[-1]
                maxlevel = findall('最大スキル Lv</span> (.+?)</p></div>', webdata)[-1]
    ...
    for idnumber in idgroup:
        ...
        rare = findall('レアリティ</span> (.+?)</p></div>', webdata)[-1]
        type = 0
        ...
        visual = findall('ビジュアル</span> (.+?)</p></div>', webdata)[-1]
        total = findall('総アピール</span> (.+?)</p></div>', webdata)[-1]
        center = findall('センター効果説明</span> (.+?)</p></div>', webdata)[-1]
        if rare == "N+":
            skill = "スキルなし"
            maxlevel = 0
        else:
    ...
    infofile.write('\n'.join(infodata))
    infofile.close()
    cls_name.config_info("Main DB and mltdkei_info.txt Update Completed.")
