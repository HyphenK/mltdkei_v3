...
- ~~업데이트 서버에 사용하는 기기 변경~~ 220616 대응 완료
- 코토리와 미사키 카드 추가로 인한 프로그램(클라이언트) 측 업데이트 코드 오류 발생
- 아래에서 언급하는 DB쪽 버그 수정
- ~~2차 송포유 스킬 관련 DB 수정~~ 220616 대응 완료, 클라이언트 측 테스트 진행중

이상의 문제로 인하여 DB 업데이트를 일시 중단합니다. 특히 두 번째는 프로그램 자체를 손봐야해서, 시간이 꽤 소요될 것으로 예상됩니다. DB 업데이트 재개 및 클라이언트 업데이트는 7월 첫주 주말 예정입니다.
