...
if __name__ == '__main__':
    freeze_support()
    root = Tk()
    root.title("MLTD Deck Analyzer 3.1 for Global-KR")
    ...
    def extra_setting():
        try:
            infofile_sldm = open('mltdkei_info_kr.txt', 'r', encoding='utf-8')
    ...
    def mltdkei_main():
        ...
        try:
            ...
            zSN = cur2.execute('''select songid from SongDB
                where name_kr = "%s"''' % str(cbxSN.get())).fetchone()[0]
        ...
        try:
            infofile_calc = open('mltdkei_info_kr.txt', 'r', encoding='utf-8')
    ...
    def update_lbPH(unused_option):
        global lbPH
        songphoto = str(cur2.execute(f'''select songphoto from SongDB where name_kr="{cbxSN.get()}"''').fetchone()[0])
        try:
    ...
    def update_cbxSN(unused_option):
        ...
        SNvalues = list()
        SNlist = cur2.execute(f'select name_kr from SongDB where type={STlc}').fetchall()
